...
async def fetch_polymarket_orderbook(client: httpx.AsyncClient, condition_id: str) -> OrderbookSnapshot:
    ...
    CATATAN: client bawaan engine TIDAK memakai proxy; karena httpx membaca
    proxy saat client DIBUAT, kita buat client lokal di dalam blok poly_proxy.
    Parameter `client` tetap diterima agar pemanggil (engine) tidak berubah.
    """
    from app.proxy_util import poly_proxy
    with poly_proxy():
        async with httpx.AsyncClient(headers=HEADERS, timeout=15.0) as pc:
            url_market = f"https://clob.polymarket.com/markets/{condition_id}"
            try:
                resp = await pc.get(url_market, headers=HEADERS, timeout=15.0)
                if resp.status_code != 200:
                    # Coba endpoint gamma sebagai fallback
                    return await _fetch_poly_fallback(pc, condition_id)
                ...
                yes_book = await _fetch_poly_book(pc, yes_token["token_id"])
                no_book = await _fetch_poly_book(pc, no_token["token_id"])
